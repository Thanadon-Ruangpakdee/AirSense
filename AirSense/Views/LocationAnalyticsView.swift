                ...
                VStack(spacing: 20) {
                    ...
                    HStack {
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            Text("\(viewModel.locationDisplayName.uppercased())")
                    ...
                    GlassCard(cornerRadius: 22, padding: 18) {
                        VStack(alignment: .leading, spacing: 14) {
                            HStack {
                                ...
                                Text("7-DAY AQI TREND (SWIFT CHARTS)")
                    ...
                    GlassCard(cornerRadius: 22, padding: 18) {
                        VStack(alignment: .leading, spacing: 16) {
                            Text("ALL GASES & PARTICULATE MATTER")
                                .font(.system(size: 11, weight: .bold, design: .rounded))
                                .foregroundColor(colorScheme == .dark ? Color.white.opacity(0.6) : Color(white: 0.45))
                                .tracking(1.2)
                            
                            PollutantProgressBar(title: "PM2.5 (Fine Particulate)", val: viewModel.currentAQIData?.pm25Value ?? 68.0, maxVal: 150, unit: "µg/m³", color: viewModel.currentSeverity.primaryColor)
                            
                            PollutantProgressBar(title: "PM10 (Coarse Particulate)", val: viewModel.currentAQIData?.pm10Value ?? 31.0, maxVal: 200, unit: "µg/m³", color: .yellow)
                            
                            PollutantProgressBar(title: "O3 (Ozone)", val: viewModel.currentAQIData?.o3Value ?? 41.1, maxVal: 100, unit: "ppb", color: .cyan)
                            
                            PollutantProgressBar(title: "NO2 (Nitrogen Dioxide)", val: viewModel.currentAQIData?.no2Value ?? 4.2, maxVal: 80, unit: "ppb", color: .orange)
                            
                            PollutantProgressBar(title: "SO2 (Sulfur Dioxide)", val: viewModel.currentAQIData?.so2Value ?? 5.1, maxVal: 50, unit: "ppb", color: .purple)
                            
                            PollutantProgressBar(title: "CO (Carbon Monoxide)", val: viewModel.currentAQIData?.coValue ?? 6.4, maxVal: 10, unit: "ppm", color: .mint)
                        }
                    }
                    .padding(.horizontal, 20)
                    .padding(.bottom, 100)
